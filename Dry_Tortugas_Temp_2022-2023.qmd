...
However, it seems like there are some rows where there is no data in the actual columns, and everything was instead recorded in the empty columns labeled "`10/10/2021`,`11`, `Loggerhead Patch`, `46`, `29.615`, `85.307`". Therefore, I will index those rows to a separate data set, which I will rename and join with the main data set to maintain the correct number of observations

```{r}
# Select all the rows that have data in the wrong columns
dt_incorrect_columns <- dt_temp_2022 %>%
  slice(22286:29704)

# Remove columns with no data 
dt_incorrect_columns <- dt_incorrect_columns %>%
  select(-c(Date, Time, Sitename, Siteid, `Temp C`, `Temp F`))
...
- Now that we have a data frame with our "missing" data, we can clean the original and bind them

```{r}
# Remove extra columns that are now empty
...
# Join the two 2022 data sets
dt_temp_2022 <- bind_rows(dt_temp_2022, dt_incorrect_columns)
```
...
```{r}
# Add the missing row that was recorded in the columns
dt_temp_2022 <- dt_temp_2022 %>%
  add_row(
    date      = as.Date("2021-10-11"),
    time      = hms::as_hms("08:00:00"),
    site_name = "Loggerhead Patch",
    site_id   = as.integer(46),
    temp_C    = 29.8,
    temp_F    = 85.6
  )
...
- Let's check now for any other strange inputs by using grepl() on our data set. Normally, we would use unique(), but seeing as how observation_id is all unique, it would give us over 97,000 unique things. So instead we will look for all values that do not contain only numbers
