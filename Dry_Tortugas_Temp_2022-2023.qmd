...
## Apply previous steps to 2024 & 2025 data
...
## Fix the 2022 data: Special Case
...
_ Next, we will create an observation_id column, look for NAs, and anomalous values
...
-   We can finally start joining this data! We will begin by creating a site_id table from the 2022 data that will be used to fill in the missing info in the 2023 to 2025 data.

```{r}
# Combine the 2022-2025 data
dt_temp_2023_2025 <- bind_rows(dt_temp_2023, dt_temp_2024, dt_temp_2025)
```

-   Next, we will check the data type of this data against the 2022 temp data

```{r}
# Check data types
str(dt_temp_2023_2025)
```

```{r}
# Check NAs or oddities
colSums(is.na(dt_temp_2023_2025))
```
- This is quite odd. I will check the number of NAs present in each data set instead





-   Ok, time to type our data following the 2006-2021 conventions!

```{r}
# Formatting the data set:

# Use as.Date on the Date column to convert 
dt_temp_2023_2025$date <- mdy(dt_temp_2023_2025$date)

# Convert to integer before applying time
dt_temp_2023_2025$time <- as.integer(dt_temp_2023_2025$time)

# Convert the integer to a hour
combined_temp$Time <- hms::hms(hours = combined_temp$Time)

# Set the siteid column as an integer
dt_temp_2023_2025$site_id <- as.integer(dt_temp_2023_2025$site_id)
```
