...
```{r}
# Make a lookup table of site_name -> site_id
site_lookup <- dt_temp_2022 %>%
  select(site_name, site_id) %>%
  distinct()  # just in case

# Join to create a site_id column
dt_temp_2023 <- dt_temp_2023 %>% left_join(site_lookup, by = "site_name")
dt_temp_2024 <- dt_temp_2024 %>% left_join(site_lookup, by = "site_name")
dt_temp_2025 <- dt_temp_2025 %>% left_join(site_lookup, by = "site_name")
```
