...
florida_keys_temp_2023_dates <- read_csv("C:/Users/Madison.Enda/Desktop/Temp_Data/florida_keys_temp_2023_dates.csv")

# Separate the dates and times

cleaning_dates <- florida_keys_temp_2023_dates %>%
  mutate(
...
fk_temp_2023 <- map_dfr(files, function(file) {
  clean_temp_csv(file) %>% 
...
- It seems as though there is one anomalous temperature recording in May of 2022 that is extremely high for 1 hour (102.214 F), and then drops back to normal. I will be removing this value before we continue.

```{r}
# Remove the anomalous value from May of 2022
fk_temp_2023 <- fk_temp_2023 %>%
  filter(temp_F < 100)
```
...
write.csv(fk_temp_2023, "florida_keys_temp_2023.csv", row.names = FALSE)
```
