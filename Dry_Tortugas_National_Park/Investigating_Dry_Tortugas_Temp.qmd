...
After a lengthy investigation, and trimming the 2022-2025 data with the updated logger deployment windows, the additional data comes from loggers recording more than the typical once per hour over a 24 hour period, or less due to logger loss, low battery, etc.


However, this all seems within the bound of possibility, and we cab therefore move forward with this data!
...
# Columns: observation_id, date, time, site_name, site_id, temp_C, temp_F
p <- ggplot(dry_tortugas_master_2025, aes(x = date, y = temp_F, 
                    group = site_name,   # group by site
                    color = site_name,   # color lines by site
                    text = paste(
                      "Site ID:", site_id,
  ...
  geom_line() +
  geom_point(alpha = 0.5) +  # points help with hover
  labs(
    x = "Date",
    y = "Temperature (F)",
