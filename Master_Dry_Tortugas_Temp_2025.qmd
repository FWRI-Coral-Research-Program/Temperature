...
```{r}
obs_2022 <-dry_tortugas_master_2025 %>%
...
dt_temp_2023 <- read_csv("C:/Users/Madison.enda/Desktop/Temp_Data/temp_dry_tortugas_2026_update/DRTO_2023_Master.csv")
...
dt_temp_2023_2025 <- bind_rows(dt_temp_2023, dt_temp_2024, dt_temp_2025)
```
...
```{r}
# Find the sum of all NA values in each column
colSums(is.na(dt_temp_2023_2025))

# Remove the added id column
dt_temp_2023_2025 <- dt_temp_2023_2025 %>%
  select(-`...1`)
```
...
```{r}
# Use bind_rows to join the data
dry_tortugas_master_2025 <- bind_rows(dry_tortugas_master_2025, dt_temp_2023_2025)
```
...
dry_tortugas_master_2025$site_name <- str_replace_all(dry_tortugas_master_2025$site_name, c(
  ...
  "Prolifer Patch" = "Prolifera Patch",
  "Prolifera" = "Prolifera Patch",
  "Temptation" = "Temptation Rock",
...
-   It seems like there are some trimming issues that did not get removed. We will assign the high temps to a filtered anomaly dataset and decide from there what should get removed.

-   We will have to ignore the high temp from 2023, where temperatures did reach insane highs, so we will filter out data above 95 from 2024 and 2025

```{r}
# Create a temperature anomaly dataframe
high_temp_dt <- dry_tortugas_master_2025 %>%
  filter(temp_F > 95)

# Remove the temperature anomalies 
dry_tortugas_master_2025 <- dry_tortugas_master_2025 %>%
filter(!(temp_F > 95 & date < as.Date("2023-08-09"))) # Remove rows where both conditions are met: temperature is higher than 95 and the date is less than August 9th, 2023. We know from looking at the high_temp_dt data set that 2022 was the only year with extraordinary anomalies for what we know the temperatures were from other sources 
```
